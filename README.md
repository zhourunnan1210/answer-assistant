...
| 📌 | 窗口置顶 / 取消置顶 |
| ⚙ | 打开设置 |
| 🎨 | 展开/收起外观面板：窗口不透明度 + 界面 UI（底板/字体/图标）+ 答案区（背景/字体/字号）的颜色与透明度，分组排布、**拖动即实时预览**、可一键恢复默认；右上角「👻 一键透明」把底板/按钮/图标全部透明化只留文字（字体颜色不变、按钮 emoji 同步去除、窗口仍可拖动、图标悬停浮现）；小窗口下面板可滚动不被压缩（与设置 → 通用 同一份配置） |
| — | 最小化到系统托盘（托盘图标右键可退出） |
