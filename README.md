...
- 📘 [设置 · 答题模型](docs/settings-model.md)：多模态模型、供应商模板、获取模型列表、思考模式、自动识别间隔、我的预设
...
- 🤖 **自动模式（v2.7）**：开启后按设定间隔（默认 30 秒，可在设置·答题模型中调整）自动识别框选区域；**混合判重**（v2.7.1）——指纹粗筛零成本跳过，画面有变化时双图发给模型仲裁，倒计时/动画噪声不会误触发重答；切换到面试模式或收进托盘自动暂停
- 🎙 **问答助手模式**：监听电脑音频输出（会议中对方的讲话），实时语音转文字，一键生成口语化回答（v2.3 起按钮暂时下线——问答是面试模式的子集，直接使用 💼 面试模式即可）
...
> **v2.3 起「🎙 问答」按钮暂时下线**：问答模式是面试模式的子集，
> 为集中保证面试模式全流程的完整与稳定，主窗口只保留「💼 面试」入口；
> 纯问答场景直接使用面试模式即可（底层监听链路相同）。
...
3. **开启监听**：点底部「💼 面试」，助手开始监听电脑的声音输出（只采系统输出，
   不采麦克风；戴耳机同样有效）
...
1. 点底部「💼 面试」开启（自动开启底层问答监听）——此时**同时监听两个通道**：
...
| 📌 | 窗口置顶 / 取消置顶 |
| ⚙ | 打开设置 |
| 🎨 | 展开/收起外观面板：窗口不透明度 + 界面 UI（底板/字体/图标）+ 答案区（背景/字体/字号）的颜色与透明度，分组排布、**拖动即实时预览**、可一键恢复默认；右上角「👻 一键透明」把底板/按钮底色/图标全部隐藏只留文字（**纯透明**：背景实时透出下层网页内容；字体颜色不变；纯图标按钮改显文字，样式与答案区一致；窗口可正常拖动）；小窗口下面板可滚动不被压缩（与设置 → 通用 同一份配置） |
| — | 最小化到系统托盘（托盘图标右键可退出） |
| ✕ | 退出程序（先隐藏窗口再后台收尾，视觉秒关） |
